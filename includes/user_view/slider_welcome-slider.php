<?php
function i3d_show_welcome_slider($sliderID) {
...
?>
<div id="bs-welcome" class="bs-animate carousel slide hidden-xs hidden-sm"  data-interval="<?php echo $slideTime; ?>">
...
foreach($portfolioImages as $image) {
	...
	//	$imageURL = str_replace("-".$large_image_url[1]."x".$large_image_url[2], "", $imageURL);
        ...
        if ($image['link'] == "") {
			...
			$imageHTML =  '<img src="'.$imageURL.'" alt="" />';
		    if ($image['link_label'] != "") {
				$linkButton = "<a class='btn btn-primary btn-lg' data-animation='animated fadeIn' target='{$image['link_target']}' href='{$linkURL}'>{$image['link_label']}</a>";
			} else {
				$linkButton = "";
			}
		}
...
<!-- slide <?php echo $imageCount; ?> -->
		<div class="<?php if ($imageCount == 1) { ?>active<?php }?> item bs-welcome-slide<?php echo $imageCount; ?>">
			<?php if (@$slider['background-overlay'] != "disabled") { ?><div class="cs_slider_image_overlay"></div><?php } ?>
			<div class="carousel-caption">
				<h2 data-animation="animated bounceInLeft"><?php echo $image['title']; ?></h2>
				<h3 data-animation="animated bounceInRight"><?php echo @$image['subtitle']; ?></h3>
				<p class="lead" data-animation="animated zoomInUp"><?php echo $image['description']; ?></p>
				<?php echo $linkButton; ?>
...
<!--/slide <?php echo $imageCount; ?> -->
		...
		<?php foreach ($slides as $slideNumber => $label) { ?>
			<li data-target="#bs-welcome" data-slide-to="<?php echo ($slideNumber - 1); ?>" <?php if ($slideNumber == 1) { ?>class="active"<?php } ?>></li>
		<?php } ?>
		</ol>
	...
	<!-- Controls -->
        <a class="left carousel-control" href="#bs-welcome" role="button" data-slide="prev">
            <span>
                <i class="fa fa-chevron-left fa-3x"></i>
            </span>
            <span class="sr-only">Previous</span>
        </a>
        <a class="right carousel-control" href="#bs-welcome" role="button" data-slide="next">
            <span>
