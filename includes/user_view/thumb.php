...
$_GET['src'] = str_replace("resize=150%2C150&", "", $_GET['src']);
			
// handle situation when the document root is very generic and not defined as we would expect
if ($_SERVER['DOCUMENT_ROOT'] == "/etc/apache2/htdocs") {
  $newRoot = array_shift(explode("/wp-content/", $_SERVER['SCRIPT_FILENAME']));
  define('LOCAL_FILE_BASE_DIRECTORY', $newRoot);
}			
...
class timthumb {
	...
	public function __construct(){
		...
		// added december 4th, 2012, to get around issue of temporary hosting spaces not being considerd local on some hosting platforms
		if (!strstr($this->src, "~")) { 
