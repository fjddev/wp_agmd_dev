...
$hasReCaptcha = false;
	...
	print "<form class='form-horizontal' method='post' ONSUBMIT>";
	...
	foreach ($form['fields'] as $fid => $field) {
	  ...
	  if ($field['fieldtype'] != "separator") { 
	  if ($field['fieldtype'] != "label" && $field['fieldtype'] != "heading"  && @$field['show_field_label'] != "0") { ?><label class='regular-label'><?php echo $field['label']; ?></label><?php } ?>
...
<?php 
	...
	?><div class='row-fluid' style='margin-top: 10px;'><input type='submit' class='btn btn-primary <?php if ($form['show_reset_button'] != "1") { ?>full-width<?php } ?>' value='<?php echo $form['submit_button_label']; ?>' /><?php 
