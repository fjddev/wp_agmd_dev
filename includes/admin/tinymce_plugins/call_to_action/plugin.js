(function() {
	...
	//tinymce.PluginManager.requireLangPack('I3DCallToAction');
	
	tinymce.create('tinymce.plugins.I3DCallToAction', {
		/**
		...
		init : function(ed, url) {
			...
			ed.addCommand('mceCallToAction', function() {			
			...
			ed.onMouseDown.add(function(ed, e) {
				if ( e.target.nodeName == 'IMG' && ed.dom.hasClass(e.target, 'i3d_cta') ) {
					t._showButtons(e.target, 'i3d_calltoaction');
		...
		_do_contact : function(co,url) {
			...
			return co.replace(/\[i3d_cta([^\]]*)\]/g, function(a,b){
				return '<img src="'+url+'/images/t.gif" class="i3d_cta mceItem" title="i3d_cta'+tinymce.DOM.encode(b)+'" />';
			});
		},
		...
		_createButtons : function() {
			...
			tinymce.dom.Event.add(editButton, 'mousedown', function(e) {
				...
				ed.execCommand("mceCallToAction");
			});
			...
			tinymce.dom.Event.add(delButton, 'mousedown', function(e) {
				...
				if ( el.nodeName == 'IMG' && ed.dom.hasClass(el, 'i3dCallToAction') ) {
					ed.dom.remove(el);
	...
	// Register plugin
	tinymce.PluginManager.add('i3d_cta', tinymce.plugins.I3DCallToAction);
})();