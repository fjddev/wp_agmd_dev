...
parse_str(str_replace('&#34;', '"', substr(preg_replace($pattern, $replacement, substr(stripslashes($_GET['properties']),16)),1)), $properties);
$pages = get_pages();
$pageOptions = "";
...
<head>
	...
	function add_content_to_display(buttonClicked) {
		...
		var checkedButton = getCheckedRadio(buttonClicked.form['i3d_form_id']);
		var formID = checkedButton.value;
		var returnStr = '<img src="<?php echo $_GET['url']; ?>/images/t.gif" class="i3d_contact_form mceItem" title="i3d_contact_form'+tinymce.DOM.encode(' id="'+formID+'"') + '" />';
		//alert(returnStr);
	   <?php
	   global $wp_version;
	...
	function selectThisRadio(radioButton) {
		...
		//alert(radioButton.form['i3d_form_id'].length);
   		for (var i = 0; i < radioButton.form['i3d_form_id'].length; i++) {
          var button = radioButton.form['i3d_form_id'][i];
          if (button.checked) {
			//  alert(button);
...
<li <?php if ($id == @$properties['id']) { echo "class='selected'"; } ?>><input onclick='selectThisRadio(this)' id="form_<?php echo $id; ?>" <?php if ($id == @$properties['id']) { print "checked"; } ?> type='radio' name="i3d_form_id" value="<?php echo $id; ?>"> <label for="form_<?php echo $id; ?>"><?php echo $form['form_title']; ?></label></li>
<?php } ?>
</ul>
...
<h3 style='clear: both;' >Create a New Form</h3>
<p>To create a new form, first save your page, then proceed to the <a target="_top" href="<?php echo get_option("siteurl") ?>/wp-admin/admin.php?page=i3d_contact_forms">contact forms creation page</a>.</p>
<input type='radio' name='i3d_form_id' value='' style='display: none;' />
