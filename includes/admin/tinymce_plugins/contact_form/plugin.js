(function() {
	...
	//tinymce.PluginManager.requireLangPack('I3DContactForm');
	
	tinymce.create('tinymce.plugins.I3DContactForm', {
		/**
		...
		init : function(ed, url) {
			...
			ed.addCommand('mceContactForm', function() {			
			...
			ed.onMouseDown.add(function(ed, e) {
				if ( e.target.nodeName == 'IMG' && ed.dom.hasClass(e.target, 'i3d_contact_form') ) {
					t._showButtons(e.target, 'i3d_contactform');
		...
		_do_contact : function(co,url) {
			...
			return co.replace(/\[i3d_contact_form([^\]]*)\]/g, function(a,b){
				return '<img src="'+url+'/images/t.gif" class="i3d_contact_form mceItem" title="i3d_contact_form'+tinymce.DOM.encode(b)+'" />';
			});
		},
		...
		_createButtons : function() {
			...
			tinymce.dom.Event.add(delButton, 'mousedown', function(e) {
				...
				if ( el.nodeName == 'IMG' && ed.dom.hasClass(el, 'i3dContactForm') ) {
					ed.dom.remove(el);
	...
	// Register plugin
	tinymce.PluginManager.add('i3d_contact_form', tinymce.plugins.I3DContactForm);
})();