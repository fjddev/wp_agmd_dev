...
tinymce.PluginManager.add('i3d_cpg', function( editor, url ) {
	...
	function replaceShortcodes( content ) {
		...
		//	tinyMCE.activeEditor.dom.loadCSS(url + '/styles.css');
			...
			return content.replace(/\[i3d_cpg([^\]]*)\]/g, function(a,b){
	...
	function restoreShortcodes( content ) {
		...
		editor.addButton('i3d_cpg', {
			classes: 'widget btn i3d_cpg',
			/*image: url + '/images/icon.png', */
			tooltip: 'Add/Modify Content Panel Group',
			cmd : 'I3D_ContentPanelGroup',
		...
		function _createButtons() {
			...
			tinymce.DOM.bind(editButton, 'mousedown', function(e) {
				var ed = tinymce.activeEditor;
							...
							ed.execCommand("I3D_ContentPanelGroup");															  
				});
			...
			tinymce.DOM.bind(delButton, 'mousedown', function(e) {
				...
				if ( el.nodeName == 'IMG' && ed.dom.hasClass(el, 'i3d_cpg') ) {
					ed.dom.remove(el);
	...
	editor.on('mousedown', function(e) {
							var t = this;	
				if ( e.target.nodeName == 'IMG' && editor.dom.hasClass(e.target, 'i3d_cpg') ) {
					_showButtons(e.target, 'i3d_contentpanelgroup');
